    ...
    return (
        <div className={style.container} onClick={() => handleClick(order._id)}>
            ...
            className={style.link}
                to={{
                    pathname: location.pathname === ROUTES.FEED ? `/feed/${order._id}` : `/profile/orders/${order._id}`,
            ...
            <div className={style.pics}>
                <div className={style.img}>
                    {orderIngredients.map((order, index) => {
                        if (index < 5) return <div className={style.img__wrapper} key={index} ><img src={order.image} alt="" /></div>
                        if (index === 5) return <div className={style.img__wrapper} key={index} ><img src={order.image} alt="" /><div className={style.count}><div className="text text_type_digits-default">+{orderIngredients.length - 5}</div></div></div>
                    })}
                </div>
