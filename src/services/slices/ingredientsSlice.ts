...
export type TIngredientsActions =
  | ReturnType<typeof fetchIngredients>
...
export const fetchIngredients = createAsyncThunk<TIngredientsThunk, void>(
  ...
  () => requestWithCheck(API_URL_INGREDIENTS)
);

export const fetchOrder = createAsyncThunk<TOrderThunk, string[]>(
  "ingredients/fetchOrder",
  (ingredients, { rejectWithValue }) =>
    fetchWithRefresh(API_URL_ORDERS, {
      method: "POST",
      mode: "cors",
...
const ingredientsSlice = createSlice({
  ...
  reducers: {
    addIngredient: (state, action) => {
      const newItem = {
        ...action.payload,
        dropId: state.lastIndexConstructor + 1,
      };
      state.constructorIngredients.push(newItem);
      state.lastIndexConstructor++;
    },
    deleteIngredient: (state, action) => {
      const ingredients = state.constructorIngredients.filter(
        (item) => item.dropId !== action.payload.dropId
      );
    ...
    total: (state) => {
      let sum = 0;
      let bunFlag = false;
      const res = state.constructorIngredients.reduce(function (
        accumulator,
        currentValue
      ) {
    ...
    setOrders: (state, action) => {
      ...
      orders &&
        orders.map((x: TOrder) => {
          ...
          let days =
            difference > 4
              ? `${difference} дней назад`
              : difference > 1
              ? `${difference} дня назад`
              : difference
              ? "Вчера"
              : "Сегодня";
          x.createdAt = `${days}, ${date.toLocaleString("ru", {
            hour: "2-digit",
            minute: "2-digit",
            timeZoneName: "shortOffset",
          })}`;
          let sum = 0;
          const orderIngredients = state.ingredients.filter((item) =>
            x.ingredients.includes(item._id)
          );
          const res = orderIngredients.reduce(function (
            accumulator,
            currentValue
          ) {
            return accumulator + currentValue.price;
